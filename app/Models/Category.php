...
{
    protected $fillable=['title','slug','summary','photo','status','is_parent','parent_id','added_by','market_id','country_id'];
    ...
    public function getCategoryInfo($categoryId)
{
    ...
    // Check if the markets relationship is not null before calling count()
    $totalMarkets = $category->markets ? $category->markets->count() : 0;

    return compact('categoryTitle', 'totalMarkets');
}
}
